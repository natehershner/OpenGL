...
glm::vec3 Factory::read_vec3(std::vector<std::string> words, glm::mat4 preTransform, float w) {
	return glm::vec3(preTransform * glm::vec4(std::stof(words[1]), std::stof(words[2]), std::stof(words[3]), w));
}

void Factory::read_corner(std::string description, std::vector<glm::vec3>& v, std::vector<glm::vec2>& vt, std::vector<glm::vec3>& vn, std::vector<float>& vertices){
...
void Factory::read_face(std::vector<std::string> words, std::vector<glm::vec3>& v, std::vector<glm::vec2>& vt, std::vector<glm::vec3>& vn, std::vector<float>& vertices) {
...
RenderComponent Factory::make_obj_mesh(const char* filename, glm::mat4 preTransform) {
	...
	while(std::getline(file, line)) {
		...
		words = split(line, " ");
		if (!words[0].compare("v")) v.push_back(read_vec3(words, preTransform, 1.0));
		else if (!words[0].compare("vt")) vt.push_back(read_vec2(words));
		else if (!words[0].compare("vn")) vn.push_back(read_vec3(words, preTransform, 0.0f)); 
		else if (!words[0].compare("f")) read_face(words, v, vt, vn, vertices);
	...
	std::cout << "Final data: " << vertices.size() << std::endl;
